...
select is((public.begin_reconciliation_run('68000000-1000-4000-8000-000000000101','inventory','fixed-export.json',repeat('a',64),'governed inventory','inventory_lot_id','1.0.0','reconciliation.test','recon-run-0001')->>'replayed'),'true','begin replay is idempotent');
...
select ok((select completed_at is not null from public.reconciliation_runs),'completed run has completion timestamp');
