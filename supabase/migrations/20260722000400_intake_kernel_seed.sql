...
-- applicability (always / conditional cross-field), requiredness, allowed
-- values, data type (text / integer / boolean / reference), category rules,
...
-- (schema.table.column) — never an EAV identity bag — and their `attr_key` is
-- that column's name. Non-identity governed bag fields carry a standalone
-- attr_key and no maps_to. `is_factual` marks facts that must never be given a
-- fabricated default: acquisition source, cost, condition, grade, grading
-- company, certificate, defects, marketplace status.
...
insert into public.intake_reference_lists (list_key, label) values
  ...
  ('source_kind', 'Stated source kind');
...
insert into public.intake_reference_options (list_key, option_value, label, sort_order) values
  ...
  -- Governed stated-source kinds. A stated source must name one of these; the
  -- bare word "stated" alone can never satisfy the source-evidence contract.
  ('source_kind', 'personal_collection', 'Personal collection', 1),
  ('source_kind', 'retail_purchase', 'Retail purchase', 2),
  ('source_kind', 'marketplace_purchase', 'Marketplace purchase', 3),
  ('source_kind', 'trade', 'Trade', 4),
  ('source_kind', 'consignment', 'Consignment', 5),
  ('source_kind', 'other', 'Other (see note)', 6);
...
insert into public.intake_field_registry
  ...
  ('tcg_set_name', 'Set name', 'product', 'set_name', 'tcg', 'text', null,
    'public.tcg_product_attributes.set_name', true, false),
  ('tcg_card_number', 'Card number', 'product', 'card_number', 'tcg', 'text', null,
    'public.tcg_product_attributes.card_number', true, false),
  ('tcg_featured_subject', 'Featured subject', 'product', 'featured_subject', 'tcg', 'text', null,
    'public.tcg_product_attributes.featured_subject', true, false),
  ('tcg_language', 'Language', 'product', 'language', 'tcg', 'reference', 'tcg_language',
    'public.tcg_product_attributes.language', true, false),
  -- TCG SKU identity (condition / grading are FACTUAL)
  ('tcg_condition_or_quality', 'Condition / quality', 'sku', 'condition_or_quality', 'tcg', 'reference', 'tcg_condition',
    'public.tcg_sku_attributes.condition_or_quality', true, true),
  ('tcg_grading_company', 'Grading company', 'sku', 'grading_company', 'tcg', 'reference', 'grading_company',
    'public.tcg_sku_attributes.grading_company', true, true),
  ('tcg_numeric_grade', 'Numeric grade', 'sku', 'numeric_grade', 'tcg', 'text', null,
    'public.tcg_sku_attributes.numeric_grade', true, true),
  ('tcg_grade_designation', 'Grade designation', 'sku', 'grade_designation', 'tcg', 'text', null,
    'public.tcg_sku_attributes.grade_designation', true, true),
  ('tcg_seal_or_packaging_condition', 'Seal / packaging condition', 'sku', 'seal_or_packaging_condition', 'tcg', 'text', null,
    'public.tcg_sku_attributes.seal_or_packaging_condition', true, true),
  ('tcg_product_format', 'Product format', 'sku', 'product_format', 'tcg', 'reference', 'tcg_product_format',
    'public.tcg_sku_attributes.product_format', true, false),
  -- TCG entry-level serialized fact (certificate is FACTUAL); maps to the item column
  ('tcg_certificate_number', 'Certificate number', 'entry', 'certificate_number', 'tcg', 'text', null,
    'public.inventory_items.certificate_number', false, true),
  -- TCG entry-level NON-identity governed bag fields (exercise integer/boolean types)
  ('tcg_case_count', 'Units in case', 'entry', 'case_count', 'tcg', 'integer', null, null, false, false),
  ('tcg_is_sealed_case', 'Sealed case?', 'entry', 'is_sealed_case', 'tcg', 'boolean', null, null, false, false),
  -- Footwear product identity
  ('footwear_silhouette', 'Silhouette', 'product', 'silhouette', 'footwear', 'text', null,
    'public.footwear_product_attributes.silhouette', true, false),
  ('footwear_colorway_name', 'Colorway', 'product', 'colorway_name', 'footwear', 'text', null,
    'public.footwear_product_attributes.colorway_name', true, false),
  ('footwear_style_code', 'Style code', 'product', 'style_code', 'footwear', 'text', null,
    'public.footwear_product_attributes.style_code', true, false),
  -- Footwear SKU identity
  ('footwear_shoe_size', 'Shoe size', 'sku', 'shoe_size', 'footwear', 'text', null,
    'public.footwear_sku_attributes.shoe_size', true, false),
  ('footwear_color', 'Color', 'sku', 'color', 'footwear', 'text', null,
    'public.footwear_sku_attributes.color', true, false),
  ('footwear_condition_or_quality', 'Condition / quality', 'sku', 'condition_or_quality', 'footwear', 'reference',
    'footwear_condition', 'public.footwear_sku_attributes.condition_or_quality', true, true);

-- Field rules ---------------------------------------------------------------------------
-- Graded TCG: grader + grade + certificate are required commit blockers.
insert into public.intake_field_rules
  ...
  -- Raw TCG: condition MAY be stated but uncertain/unknown is permitted.
  ('raw_tcg', 'tcg_condition_or_quality', 'always', false, false, '{}'),
  ('raw_tcg', 'tcg_featured_subject', 'always', false, false, '{}'),
  -- Sealed TCG: product format meaningful; and a CONDITIONAL rule — seal /
  -- packaging condition is a required blocker ONLY when the sealed group is
  -- explicitly expanded into serialized units (per-unit seal state matters then).
  ('sealed_tcg', 'tcg_product_format', 'always', false, false, '{}'),
  ('sealed_tcg', 'tcg_seal_or_packaging_condition', 'conditional', true, true,
