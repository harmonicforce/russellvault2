...
alter table public.audit_events add constraint audit_events_event_type_check check (event_type in (
  'source_system_registered','import_previewed','import_started','import_records_staged',
  'import_committed','import_failed','source_record_ingested','crosswalk_candidate_created',
  'crosswalk_confirmed','crosswalk_rejected','crosswalk_superseded','issue_opened',
  'issue_acknowledged','issue_resolved','issue_wont_fix','channel_registered',
  'supplier_registered','supplier_alias_created','acquisition_import_started',
  'acquisition_import_staged','acquisition_import_committed','acquisition_import_failed',
  'lot_line_superseded','cost_component_reversed','cost_allocation_proposed',
  'cost_allocation_confirmed','cost_allocation_reversed','acquisition_line_classified',
  'acquisition_line_classification_superseded','acquisition_line_classification_overridden',
  'classification_rule_created','classification_rule_superseded'
));
...
begin
  ...
  left join public.supplier_aliases a on a.supplier_id = o.supplier_id
    and a.workspace_id = o.workspace_id
    and a.source_system_id = o.source_system_id
    and a.raw_handle = coalesce(
      li.source_detail->>'seller_raw_handle',
      osr.raw_payload->>'seller',
      osr.parser_output->>'seller'
    )
...
create function app.classification_regex_matches(p_value text, p_pattern text, p_flags text)
returns boolean language sql immutable set search_path='' as $$
  -- Seeded patterns preserve the JavaScript classifier source. PostgreSQL ARE
  -- spells the zero-width word boundary \y rather than JavaScript's \b.
  select coalesce(regexp_like(p_value, replace(p_pattern, E'\\b', E'\\y'), coalesce(p_flags, '')), false);
$$;
revoke all on function app.classification_regex_matches(text,text,text) from public;
...
as $$
declare
  v_input jsonb; v_match_count int; v_rule public.classification_rules%rowtype; v_option public.acquisition_classification_options%rowtype;
  v_is_card boolean;
begin
  v_input := app.get_acquisition_classification_input(p_acquisition_line_item_id);
  v_is_card := coalesce(v_input->>'business_vertical', '') = 'Pokémon / TCG';

  -- Non-card lines are classified by their business vertical before title
  -- inspection. Unknown non-card verticals intentionally become Other.
  if not v_is_card then
    select count(*) into v_match_count
    from public.classification_rules r
    where r.workspace_id = (v_input->>'workspace_id')::uuid
      and r.status = 'active'
      and r.rule_family = 'business_vertical_mapping'
      and r.matcher_kind = 'exact'
      and app.classification_match_value(v_input, r.match_field) = r.exact_value;
    if v_match_count > 1 then
      raise exception 'ambiguous acquisition classification rules for winning precedence' using errcode='23514';
    end if;
    if v_match_count = 0 then
      select * into v_option from public.acquisition_classification_options
      where workspace_id=(v_input->>'workspace_id')::uuid and key='other' and active;
      if v_option.id is null then raise exception 'active other classification option is required' using errcode='23514'; end if;
      return jsonb_build_object('workspace_id',v_option.workspace_id,'acquisition_line_item_id',p_acquisition_line_item_id,'classification_option_id',v_option.id,'option_key',v_option.key,'option_label',v_option.label,'rule_id',null,'rule_public_id',null,'rule_version',null,'logical_key',null,'rule_family',null,'method','system_fallback','confidence',1.0000,'system_provenance','governed_acquisition_classifier_s1_2','evidence',jsonb_build_object('input',v_input,'fallback','unknown_non_card_vertical','explicit_evidence_status','legacy_sealed_line_ids_unavailable'));
    end if;
  ...
  with matched as (
    ...
    where r.workspace_id = (v_input->>'workspace_id')::uuid and r.status='active'
      ...
      and case r.matcher_kind
        ...
        when 'regex' then app.classification_regex_matches(app.classification_match_value(v_input,r.match_field), r.pattern, r.pattern_flags)
        when 'evidence_set' then false
        else false end
  ), winning as (select * from matched where precedence = (select min(precedence) from matched))
  select count(*) into v_match_count from winning;

  if v_match_count > 1 then
    raise exception 'ambiguous acquisition classification rules for winning precedence' using errcode='23514';
  end if;
  if v_match_count = 1 then
    select r.* into v_rule from public.classification_rules r
      where r.workspace_id=(v_input->>'workspace_id')::uuid and r.status='active'
        and r.rule_family <> 'business_vertical_mapping'
        and case r.matcher_kind
          when 'exact' then app.classification_match_value(v_input,r.match_field)=r.exact_value
        ...
        and r.precedence = (select min(r2.precedence) from public.classification_rules r2
          where r2.workspace_id=(v_input->>'workspace_id')::uuid and r2.status='active'
            and r2.rule_family <> 'business_vertical_mapping'
            and case r2.matcher_kind
              when 'exact' then app.classification_match_value(v_input,r2.match_field)=r2.exact_value
              when 'regex' then app.classification_regex_matches(app.classification_match_value(v_input,r2.match_field), r2.pattern, r2.pattern_flags)
              when 'evidence_set' then false else false end);
  end if;
  end if;

  if v_rule.id is not null then
    select * into v_option from public.acquisition_classification_options where id=v_rule.target_classification_option_id and workspace_id=v_rule.workspace_id and active;
    if v_option.id is null then raise exception 'active target classification option is required' using errcode='23514'; end if;
...
alter table public.acquisition_line_classifications drop constraint acquisition_line_classifications_method_check;
alter table public.acquisition_line_classifications add constraint acquisition_line_classifications_method_check
  check (method in ('rule','owner_override','seller_specialization','explicit_evidence','system_fallback'));
alter table public.acquisition_line_classifications drop constraint acquisition_line_classifications_method_actor;
alter table public.acquisition_line_classifications add constraint acquisition_line_classifications_method_actor check (
  ...
  or (method = 'system_fallback' and created_by is null and system_provenance is not null and rule_id is null and rule_version is null)
  or (method in ('rule','seller_specialization','explicit_evidence') and created_by is null and system_provenance is not null)
);
...
alter table public.acquisition_line_classifications add constraint acquisition_line_classifications_rule_presence check (
  ...
  or (method in ('owner_override','system_fallback') and rule_id is null and rule_version is null)
);
