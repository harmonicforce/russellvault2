...
declare u uuid; x public.reconciliation_runs%rowtype; fp text; key text:=btrim(p_idempotency_key); begin
 ...
 fp:=encode(sha256(concat_ws('|',p_domain,p_source_label,p_source_sha256,p_target_scope,p_comparison_key,p_tool_version,p_actor_process)::bytea),'hex');
 ...
 values(p_workspace_id,app.mint_governed_public_id('RV-RECON'),btrim(p_domain),btrim(p_source_label),p_source_sha256,btrim(p_target_scope),btrim(p_comparison_key),btrim(p_tool_version),u,p_actor_process,key,fp) returning * into x;
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare u uuid; f public.reconciliation_findings%rowtype; x public.reconciliation_finding_adjudications%rowtype; fp text; key text:=btrim(p_idempotency_key); begin
 u:=app.assert_workspace_role(p_workspace_id,array['owner']::public.workspace_role[]);
 if key is null or char_length(key) not between 8 and 200 or (p_state<>'open' and nullif(btrim(p_note),'') is null) then raise exception 'invalid_request' using errcode='22023'; end if;
 ...
 fp:=encode(sha256(concat_ws('|',f.id,p_state,p_note,p_actor_process)::bytea),'hex'); perform pg_advisory_xact_lock(hashtextextended(p_workspace_id::text||key,0));
 ...
 values(p_workspace_id,app.mint_governed_public_id('RV-RECONA'),f.id,p_state,nullif(btrim(p_note),''),u,key,fp,p_actor_process) returning * into x;
