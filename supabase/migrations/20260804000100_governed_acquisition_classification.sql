...
create table public.classification_rules (
  ...
  foreign key (target_classification_option_id, workspace_id) references public.acquisition_classification_options (id, workspace_id) on delete restrict,
  foreign key (supersedes_rule_id, workspace_id) references public.classification_rules (id, workspace_id) on delete restrict,
...
create table public.acquisition_line_classifications (
  ...
  foreign key (classification_option_id, workspace_id) references public.acquisition_classification_options (id, workspace_id) on delete restrict,
  foreign key (rule_id, workspace_id) references public.classification_rules (id, workspace_id) on delete restrict,
  foreign key (supersedes_classification_id, workspace_id) references public.acquisition_line_classifications (id, workspace_id) on delete restrict,
...
create unique index acquisition_line_classifications_one_current_uidx on public.acquisition_line_classifications (workspace_id, acquisition_line_item_id) where supersedes_classification_id is null;
...
begin
 if tg_op='DELETE' then raise exception 'classification rules are append-only' using errcode='2F000'; end if;
...
begin
 if tg_op='DELETE' then raise exception 'acquisition classifications are append-only' using errcode='2F000'; end if;
