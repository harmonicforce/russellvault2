...
router.post('/', (req, res) => {
  ...
  const quantitySold = Number(b.quantity_sold) || 1;
  const available = Number(lot.available_quantity) || 0;
  if (quantitySold > available) {
    return res.status(400).json({ error: `only ${available} unit(s) available for this lot` });
  }
  ...
  const row = db.prepare('SELECT * FROM sales WHERE sale_id = ?').get(saleId);
