...
Governed acquisition line decisions are append-only rows in `acquisition_line_classifications`. A newer decision points at the prior decision through `supersedes_classification_id`, so the current row and historical rows remain queryable without mutating old semantic evidence. Until later S1 cutover work, `server/src/classify.ts` remains the active classifier for the legacy SQLite path only; no Express endpoint or React page reads the governed classification tables yet.
