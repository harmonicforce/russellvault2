...
4. **The exclusion decision ledger shares the transaction-timestamp ordering caveat** noted for shipment transitions: `created_at` defaults to `now()`, so decisions written in one transaction share an instant. Test 63 orders history deterministically because its decisions land in separate statements with `clock_timestamp()` supersession, but a future batch operation would need a sequence.
